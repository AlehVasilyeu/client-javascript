...
# ReportPortal Client for Javascript

This Client is to communicate with the Report Portal on Node.js.

Library is used only for implementors of custom listeners for ReportPortal. Able to work both on Node.js and plain Javascript (for this pass module through Babel with correct preset, for bundling you can use Webpack).
...
| ----------- | -------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| token       | yes      | ''               | User's token Report Portal from which you want to send requests. It can be found on the profile page of this user.                               |
| endpoint    | yes      | ''               | URL of your server. For example, if you visit the page at 'https://server:8080/ui', then endpoint will be equal to 'https://server:8080/api/v1'. |
| launch      | yes      | Test launch name | Name of launch at creation.                                                                                                                      |
| project     | yes      | ''               | The name of the project in which the launches will be created.                                                                                   |
| description | no       | ''               | Additional description which you can add to your launch.                                                                                         |
| tags        | no       | []               | Tags could be added to launch to help identify particular launch.                                                                                |
| debug       | no       | false            | Turns on debug messages.                                                                                                                         |
| concurrency | no       | 1                | Specifies how much requests in parallel could work.                                                                                              |
...
```javascript
{
    // Generated by the client id for further work with the created item, 
    // this temp id you can use for managing specific items in future, 
    // first time it will created by any 'start^' methods,
    tempId: '4ds43fs', 
    // An object indicating the completion of an operation.
    promise: Promise 
}
```
