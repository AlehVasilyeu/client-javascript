...
class RPClient {
    ...
    getNewItemObj() {
        const obj = {
            ...
            childrens: [],
            finishSend: false,
            active: false, // checks whether promise for this completed
        };
        return obj;
    }
    ...
    startLaunch(launchDataRQ) {
        ...
        const tempLaunchItem = this.tree.addItem(cleanItemObj);
        let promise = null;

        if (launchDataRQ.id) {
            tempLaunchItem.realId = launchDataRQ.id;
            promise = Promise.resolve({ id: launchDataRQ.id });
        } else {
            const launchData =
                Object.assign(
                    {
            ...
            log.debug(`Start launch ${tempLaunchItem.id}`);
            promise = schedule(this.queue, () => {
                tempLaunchItem.active = true;
                return this.restClient.create(url, launchData, { headers: this.headers })
                    .then((response) => {
                        log.debug(`Success start launch ${response.id}`);
                        tempLaunchItem.active = false;
                        tempLaunchItem.realId = response.id;
                        return response;
                    }, (error) => {
                        tempLaunchItem.active = false;
                        log.debug(`Error start launch ${tempLaunchItem.id}`);
    ...
    finishLaunch(launchTempId, finishExecutionRQ) {
        return {
            ...
            promise: schedule(this.queue, () => {
                ...
                const url = [this.config.project, 'launch', launchObj.realId, 'finish'].join('/');
    ...
    updateLaunch(launchTempId, launchData) {
        return {
            ...
            promise: schedule(this.queue, () => {
                ...
                const url = [this.config.project, 'launch', launchObj.realId, 'update'].join('/');
        ...
        return {
            ...
            promise: schedule(this.queue, () => {
                ...
                if (launchObj.finishSend) {
                    ...
                    return Promise.reject(new Error(`Launch "${launchTempId}" is already finished, you can not add an item to it`));
                }
                ...
                this.map[tempId] = this.getNewItemObj();
    ...
    finishTestItem(itemTempId, finishTestItemRQ) {
        return {
            ...
            promise: schedule(this.queue, () => {
                ...
                if (!itemObj) {
                    return Promise.reject(new Error(`Item "${itemTempId}" not found`));
                }
                ...
                return this.finishTestItemPromiseStart(itemObj, itemTempId, finishTestItemData);
            }),
        };
    }

    saveLog(itemTempId, requestPromiseFunc) {
        const tempId = uniqid();
        return {
            tempId,
            promise: schedule(this.queue, () => {
                ...
                this.map[tempId] = this.getNewItemObj();
                log.debug(`Save log ${tempId}`);
                return requestPromiseFunc(itemObj.realId)
                    .then((response) => {
                        log.debug(`Successfully save log ${tempId}`);
                        // resolve(response);
    ...
    finishTestItemPromiseStart(itemObj, itemTempId, finishTestItemData) {
        const url = [this.config.project, 'item', itemObj.realId].join('/');
        log.debug(`Finish test item ${itemTempId}`);
        return this.restClient.update(url, finishTestItemData, { headers: this.headers })
            .then((response) => {
                log.debug(`Success finish item ${itemTempId}`);
                return response;
            }, (error) => {
                log.debug(`Error finish test item ${itemTempId}`);
                log.debug(error);
                throw new Error(error);
            });
    }
