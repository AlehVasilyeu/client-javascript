...
class RPClient {
    ...
    waitForRealId(tempItemId, interval = 100) {
        console.log('wait for realId');
        const tempItem = this.tree.getItem(tempItemId);
        if (!tempItem) {
    ...
    waitForItemChildrenCompleted(tempItemId, interval = 100) {
        ...
        return helpers.waitForCondition(
            () => {
                // console.log(tempItem);
                return !this.isActiveChildren(tempItemId);
            },
            interval,
            300,
        );
    ...
    waitForItemCompleted(tempItemId, interval = 100) {
        ...
        return helpers.waitForCondition(
            () => {
                // console.log(tempItem);
                return !this.isActive(tempItemId);
            },
            interval,
            300,
        );
    ...
    finishLaunch(launchTempId, finishExecutionRQ) {
        ...
        // launchTempItem.active = true;
        launchTempItem.promise = this.getPromiseFinishAllItems(launchTempId)
            .then(schedule(this.queue, () => {
                return this.waitForItemCompleted(launchTempId)
                    // .then(() => this.getPromiseFinishAllItems(launchTempId))
                    .then(() => {
    ...
    startTestItem(testItemDataRQ, launchTempId, parentTempId) {
        ...
        testItemTempItem.promise = this.waitForItemCompleted(parentTempId || launchTempId)
            .then(() => {
            ...
            .then(() => schedule(this.queue, () => {
                ...
                return this.restClient.create(url, testItemData, { headers: this.headers })
                    .then((response) => {
                        ...
                        log.debug(`[OK] Success start item ${testItemDataRQ.type} ${testItemTempId} ${parentId}`);
    ...
    finishTestItem(testItemTempId, finishTestItemRQ) {
        ...
        // testItemTempItem.active = true;
        testItemTempItem.promise = this.waitForItemCompleted(testItemTempId)
            .then(() => {
            ...
            .then(() => {
                ...
                return this.waitForItemChildrenCompleted(testItemTempId)
                    ...
                    .then(() => this.finishTestItemPromiseStart(testItemTempItem, testItemTempId, finishTestItemData));
    ...
    saveLog(testItemTempId, requestPromiseFunc) {
        ...
        logItemTempItem.promise = this.waitForRealId(testItemTempId)
            .then(() => this.waitForItemCompleted(testItemTempId))
            .then(() => {
    ...
    finishTestItemPromiseStart(testItemTempItem, testItemTempId, finishTestItemData) {
        ...
        testItemTempItem.active = true;
        return schedule(this.queue, () => this.restClient.update(url, finishTestItemData, { headers: this.headers })
            .then((response) => {
                testItemTempItem.active = false;
