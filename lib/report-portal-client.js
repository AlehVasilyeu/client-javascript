...
const STATE_CHECK_MAX_ATTEMPTS = 300;
const STATE_CHECK_INTERVAL = 100;
...
class RPClient {
    ...
    waitForItemChildrenCompleted(tempItemId, interval = STATE_CHECK_INTERVAL) {
        const tempItem = this.tree.getItem(tempItemId);
        if (!tempItem) {
        ...
        return helpers.waitForCondition(
            ...
            interval,
            STATE_CHECK_MAX_ATTEMPTS,
        );
    }

    waitForItemCompleted(tempItemId, interval = STATE_CHECK_INTERVAL) {
        const tempItem = this.tree.getItem(tempItemId);
        if (!tempItem) {
            return promisifyedError(`Item "${tempItemId}" not found`);
        }
    ...
    checkConnect() {
        return this.schedule(() => this.restClient.retrieve('user', { headers: this.headers }));
    }
    ...
    saveLog(testItemTempId, requestPromiseFunc) {
        ...
        try {
            this.tree.addItem(cleanItemObj, testItemTempId);
        } catch (e) {
            return {
                tempId: logItemTempId,
                promise: promisifyedError(e),
            };
        }
    ...
    finishTestItemPromiseStart(testItemTempItem, testItemTempId, finishTestItemData) {
        ...
        return this.restClient.update(url, finishTestItemData, { headers: this.headers })
            ...
            .then((response) => {
                this.tree.removeAllChildren(testItemTempId);
                return response; // pass again response;
            });
    }
}
